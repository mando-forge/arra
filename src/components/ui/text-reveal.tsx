  ...
  return (
    <div ref={sectionRef} className={cn("relative z-0 h-[200vh]", className)}>
      <div className="sticky top-0 mx-auto flex h-[50%] max-w-5xl items-center bg-transparent px-4 py-20">
        <span className="flex flex-wrap p-5 text-2xl font-bold md:p-8 md:text-3xl lg:p-10 lg:text-4xl xl:text-5xl">
          {words.map((word, i) => {
...
const Word: FC<WordProps> = ({ children, progress, range }) => {
  ...
  return (
    <span className="relative mx-1 lg:mx-1.5">
      {/* Ghost — always visible as a dim placeholder */}
      <span className="absolute text-foreground/25">{children}</span>
      {/* Revealed text fades in as user scrolls */}
